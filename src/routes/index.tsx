    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center transition-all duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`}
      ...
      <div className={`relative w-full max-w-sm mx-4 rounded-2xl overflow-hidden shadow-2xl transition-all duration-300 ${visible ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}
        ...
        <div className="p-5 grid grid-cols-2 gap-2">
          <button onClick={shareTwitter} className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-opacity hover:opacity-90" style={{ background: '#000' }}>𝕏 Twitter</button>
          <button onClick={shareFacebook} className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-opacity hover:opacity-90" style={{ background: '#1877F2' }}>Facebook</button>
          <button onClick={shareWhatsApp} className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-opacity hover:opacity-90" style={{ background: '#25D366' }}>WhatsApp</button>
          <button onClick={shareTelegram} className="flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-bold text-white transition-opacity hover:opacity-90" style={{ background: '#2AABEE' }}>Telegram</button>
        </div>
        ...
        <div className="mx-auto w-full max-w-3xl px-4 pb-2">
          <div className="rounded-xl border px-4 py-3 text-sm"
            ...
            <div className="mb-1 flex items-center gap-2 font-semibold" style={{ color: '#C084FC' }}>
              ▶ Video URL detected
              <button onClick={() => { setVideoContextPending(null); setInput('') }}
                className="ml-auto text-xs opacity-60 hover:opacity-100" style={{ color: '#AFA9EC' }}>
                ✕ Cancel
              </button>
            </div>
            <div style={{ color: '#AFA9EC' }}>What specific claim or topic should I fact-check in this video?</div>
            <div className="mt-1 truncate text-xs" style={{ color: '#7F77DD' }}>{videoContextPending}</div>
          </div>
        </div>
      )}
